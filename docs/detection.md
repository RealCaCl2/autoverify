...
| # | 方式 | 说明 | 本部署状态 |
|---|---|---|---|
| 14 | **IPv6 泄露** | 若客户端拿到全局 IPv6 地址，**绕过 NAT，每台设备地址直接暴露**，前面所有工作白做 | ✅ 当前无泄露（LAN 只有 ULA `fd16:2fde:6eca::/60`，WAN 无 v6 地址、无 v6 默认路由）。⚠️ 但 `ra='server'` + `ra_slaac='1'` 是开着的，现在安全仅仅因为没有全局前缀 |
...
| 17 | 路由器自身指纹 | 路由器自己的 DHCP 客户端指纹、出站 UA 等 | ⚠️ 部分处理：**出站 UA 已换成普通 Chrome**（`tuning.user_agent`；旧值形如 `(Linux; OpenWrt) autoverify`，等于每次探测都自报家门）。但 **DHCP 客户端指纹仍会暴露这是一台 OpenWrt 路由器** |
...
| 厂商 | 声称的能力 |
|---|---|
| 深澜 防代理系统 | 自研算法 + 深度包分析 + 设备行为模型；分级处置"提醒 → 降速 → 下线 → 封禁" |
