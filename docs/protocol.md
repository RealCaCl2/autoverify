...
>
> 实测环境：淮安大学（原淮阴工学院，2026-02 经教育部批准更名）
> `iHuaiGong-Student`，卓智/深澜 zportal。
...
> 这段是**抓包的原始值，原样保留**：`HYGXY`（原淮阴工学院拼音缩写）和 `hyit.edu.cn`
> 都是 NAS 配置里的旧值。学校 2026-02 已更名为淮安大学（新域名为 `hau.edu.cn`），
> 但 NAS 侧未必同步。**这些参数全部按 NAS 给的原样回填**，程序不猜不改。
